...
def register(pkg_name, version, author, short_desc, long_desc, homepage, link):
    ...
    template = template.replace("__package_name", pkg_name)
    template = template.replace("__version", version)
    template = template.replace(
        "__link", "{}#egg={}-{}".format(link, norm_pkg_name, version)
    )
    template = template.replace("__homepage", homepage)
    template = template.replace("__author", author)
    template = template.replace("__long_description", long_desc)
